    ...
    public static class PortalBlock extends Block {
        ...
        public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
            if (!entityIn.isPassenger() && !entityIn.isBeingRidden() && entityIn.isNonBoss()) {
                if (entityIn.dimension != DimensionType.byName(TestMod.EXAMPLE_DIM_TYPE)) {
                    teleportToDimension(entityIn, DimensionType.byName(TestMod.EXAMPLE_DIM_TYPE), pos);
                } else {
                ...
                if (entityIn.timeUntilPortal > 0) {
                    ...
                    if (!entityIn.world.isRemote) {
                        ...
                        BlockPattern.PatternHelper blockpattern$patternhelper = NetherPortalBlock
                                .createPatternHelper(entityIn.world, lastPortalPos);
                        ...
                        Vec3d lastPortalVec = new Vec3d(d1, d2, 0.0D);
                        Direction teleportDirection = blockpattern$patternhelper.getForwards();
                    }
